...
def findItemSuggestions(shopping_instances):
    ...
    frequent_itemsets = apriori(store, min_support = 0.001, use_colnames = True)
    
    # extract support for single items
    single_items = frequent_itemsets[frequent_itemsets['itemsets'].apply(len) == 1]
    single_items.insert(0, "single", [list(tup)[0] for tup in single_items["itemsets"]], False)
    ...
    single_rules = association_rules[(association_rules["antecedents"].apply(len) == 1) & (association_rules["consequents"].apply(len) == 1)]
    single_rules.insert(0,"antecedent", [list(tup)[0] for tup in single_rules["antecedents"]], True)
    single_rules.insert(1,"consequent", [list(tup)[0] for tup in single_rules["consequents"]], True)
    ...
    for index, rule in single_rules.iterrows():
        Association.create(rule["antecedent"], rule["consequent"], rule["support"], rule["confidence"], rule["lift"])
